...
// City Selection
import 'package:flutter_dozor_city/features/city_selection/domain/usecases/check_city_data_freshness_use_case.dart' as city_selection;
...
abstract final class FeaturesModule {
  static void register() {
    // City Selection
    injector.registerFactory(() => GetCitiesUseCase(injector<CityRepository>()));
    injector.registerFactory(() => city_selection.CheckCityDataFreshnessUseCase(injector<CityRepository>()));
    injector.registerFactory(() => SelectCityUseCase(
      cityRepository: injector<CityRepository>(),
      sessionRepository: injector<SessionRepository>(),
      checkCityDataFreshnessUseCase: injector<city_selection.CheckCityDataFreshnessUseCase>(),
    ));
    injector.registerFactory(() => CitySelectionCubit(
      getCitiesUseCase: injector(),
      selectCityUseCase: injector(),
    ));

    // Live Tracking
    injector.registerFactory(() => GetCityVehiclesUseCase(injector<CityRepository>()));
    injector.registerFactory(() => LiveTrackingCubit(
    ...
    // Main Map
    injector.registerFactory(() => GetSelectedCityUseCase(injector<SessionRepository>()));
    injector.registerFactory(() => GetMapCameraUseCase(injector<SessionRepository>()));
    injector.registerFactory(() => SaveMapCameraUseCase(injector<SessionRepository>()));
    injector.registerFactory(() => GetUiFlagUseCase(injector<SessionRepository>()));
    injector.registerFactory(() => SetUiFlagUseCase(injector<SessionRepository>()));
    injector.registerFactory(() => GetSelectedMapRoutesUseCase(injector<SessionRepository>()));
    injector.registerFactory(() => SaveSelectedMapRoutesUseCase(injector<SessionRepository>()));
    injector.registerFactory(() => CheckMainMapCityDataFreshnessUseCase(injector<CityRepository>()));
    
    injector.registerFactory(() => MainMapCubit(
    ...
    // Map Routes & Arrivals
    injector.registerFactory(() => GetRoutesByTypeUseCase(injector<CityRepository>()));
    injector.registerFactory(() => GetRouteZonesUseCase(injector<CityRepository>()));
    injector.registerFactory(() => GetArrivalByZoneUseCase(injector<CityRepository>()));
    
    injector.registerFactory(() => MapRoutesCubit(
      getRoutesByTypeUseCase: injector(),
      getSelectedMapRoutesUseCase: injector(),
      saveSelectedMapRoutesUseCase: injector(),
    ));
    
    injector.registerFactory(() => MapArrivalsCubit(
      getRouteZonesUseCase: injector(),
      getArrivalByZoneUseCase: injector(),
      pollingScheduler: injector<PollingScheduler>(),
    ));

    injector.registerFactory(() => MapOverlaysCubit(
      getRoutesByTypeUseCase: injector(),
      getRouteZonesUseCase: injector(),
      getArrivalByZoneUseCase: injector(),
      pollingScheduler: injector<PollingScheduler>(),
    ...
    // Point Select
    injector.registerFactory(() => GetCurrentLocationUseCase(injector<SearchRepository>()));
    injector.registerFactory(() => SearchAddressSuggestionsUseCase(injector<SearchRepository>()));
    injector.registerFactory(() => PointSelectCubit(
      getCurrentLocationUseCase: injector(),
      searchAddressSuggestionsUseCase: injector(),
    ));

    // Route Preview
    injector.registerFactory(() => RoutePreviewCubit());

    // Route Results
    injector.registerFactory(() => SearchRoutesUseCase(injector<SearchRepository>()));
    injector.registerFactory(() => RouteResultsCubit(
    ...
    // Route Search
    injector.registerFactory(() => LoadSearchDraftUseCase(injector<SearchDraftRepository>()));
    injector.registerFactory(() => SaveSearchDraftUseCase(injector<SearchDraftRepository>()));
    ...
    injector.registerFactory(() => RouteSearchCubit(
    ...
    // Stored Routes
    injector.registerFactory(() => GetStoredRoutesUseCase(injector<StoredRoutesRepository>()));
    injector.registerFactory(() => WatchStoredRoutesUseCase(injector<StoredRoutesRepository>()));
    injector.registerFactory(() => DeleteStoredRouteUseCase(injector<StoredRoutesRepository>()));
    injector.registerFactory(() => ToggleStoredRouteUseCase(injector<StoredRoutesRepository>()));
    injector.registerFactory(() => StoredRoutesCubit(
      getStoredRoutesUseCase: injector(),
      watchStoredRoutesUseCase: injector(),
